...
public final class Fort extends AbstractResidence {
	...
	public class FortFunction {
		...
		public void dbSave() {
			try (var con = ConnectionFactory.getInstance().getConnection();
				var ps = con.prepareStatement("REPLACE INTO fort_functions (fort_id, type, lvl, lease, rate, endTime) VALUES (?,?,?,?,?,?)")) {
	...
	public void raiseSupplyLvL() {
		_supplyLvL++;
		if (_supplyLvL > fortress().getMaxSupplyLevel()) {
			_supplyLvL = fortress().getMaxSupplyLevel();
		}
	}
	
	public void setSupplyLvL(int val) {
		if (val <= fortress().getMaxSupplyLevel()) {
			_supplyLvL = val;
		}
	}
	...
	public void saveFortVariables() {
		try (var con = ConnectionFactory.getInstance().getConnection();
			var ps = con.prepareStatement("UPDATE fort SET supplyLvL=? WHERE id = ?")) {
	...
	protected void load() {
		try (var con = ConnectionFactory.getInstance().getConnection();
			...
			if (ownerId > 0) {
				...
				while (initial > fortress().getPeriodicUpdateFrequency()) {
					initial -= fortress().getPeriodicUpdateFrequency();
				}
	...
	private void loadFunctions() {
		try (var con = ConnectionFactory.getInstance().getConnection();
			var ps = con.prepareStatement("SELECT * FROM fort_functions WHERE fort_id = ?")) {
			ps.setInt(1, getResidenceId());
			try (var rs = ps.executeQuery()) {
				while (rs.next()) {
	...
	private void loadDoorUpgrade() {
		try (var con = ConnectionFactory.getInstance().getConnection();
			var ps = con.prepareStatement("SELECT * FROM fort_doorupgrade WHERE fortId = ?")) {
			ps.setInt(1, getResidenceId());
			try (var rs = ps.executeQuery()) {
				while (rs.next()) {
	...
	private void removeDoorUpgrade() {
		try (var con = ConnectionFactory.getInstance().getConnection();
			var ps = con.prepareStatement("DELETE FROM fort_doorupgrade WHERE fortId = ?")) {
			ps.setInt(1, getResidenceId());
			ps.execute();
		} catch (Exception e) {
	...
	private void saveDoorUpgrade(int doorId, int hp, int pDef, int mDef) {
		try (var con = ConnectionFactory.getInstance().getConnection();
			var ps = con.prepareStatement("INSERT INTO fort_doorupgrade (doorId, hp, pDef, mDef) VALUES (?,?,?,?)")) {
			ps.setInt(1, doorId);
			ps.setInt(2, hp);
			ps.setInt(3, pDef);
			ps.setInt(4, mDef);
	...
	private void updateOwnerInDB() {
		...
		try (var con = ConnectionFactory.getInstance().getConnection();
			var ps = con.prepareStatement("UPDATE fort SET owner=?,lastOwnedTime=?,state=?,castleId=? WHERE id = ?")) {
			ps.setInt(1, clanId);
	...
	public void updateClansReputation(L2Clan owner, boolean removePoints) {
		if (owner != null) {
			if (removePoints) {
				owner.takeReputationScore(clan().getLoseFortPoints(), true);
			} else {
				owner.addReputationScore(clan().getTakeFortPoints(), true);
			}
		}
	}
	...
	public final void setFortState(int state, int castleId) {
		...
		_castleId = castleId;
		try (var con = ConnectionFactory.getInstance().getConnection();
			var ps = con.prepareStatement("UPDATE fort SET state=?,castleId=? WHERE id = ?")) {
	...
	private void initNpcs() {
		try (var con = ConnectionFactory.getInstance().getConnection();
			var ps = con.prepareStatement("SELECT * FROM fort_spawnlist WHERE fortId = ? AND spawnType = ?")) {
			ps.setInt(1, getResidenceId());
			ps.setInt(2, 0);
			try (var rs = ps.executeQuery()) {
				while (rs.next()) {
	...
	private void initSiegeNpcs() {
		_siegeNpcs.clear();
		try (var con = ConnectionFactory.getInstance().getConnection();
			var ps = con.prepareStatement("SELECT id, npcId, x, y, z, heading FROM fort_spawnlist WHERE fortId = ? AND spawnType = ? ORDER BY id")) {
			ps.setInt(1, getResidenceId());
			ps.setInt(2, 2);
			try (var rs = ps.executeQuery()) {
				while (rs.next()) {
	...
	private void initSpecialEnvoys() {
		...
		try (var con = ConnectionFactory.getInstance().getConnection();
			var ps = con.prepareStatement("SELECT id, npcId, x, y, z, heading, castleId FROM fort_spawnlist WHERE fortId = ? AND spawnType = ? ORDER BY id")) {
			ps.setInt(1, getResidenceId());
			ps.setInt(2, 3);
			try (var rs = ps.executeQuery()) {
				while (rs.next()) {
