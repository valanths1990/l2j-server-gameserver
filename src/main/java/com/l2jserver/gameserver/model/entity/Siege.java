...
public class Siege implements Siegable {
	...
	public void clearSiegeClan() {
		try (var con = ConnectionFactory.getInstance().getConnection();
			var ps = con.prepareStatement("DELETE FROM siege_clans WHERE castle_id=?")) {
			ps.setInt(1, getCastle().getResidenceId());
			ps.execute();
			
			if (getCastle().getOwnerId() > 0) {
				try (var delete = con.prepareStatement("DELETE FROM siege_clans WHERE clan_id=?")) {
	...
	public void clearSiegeWaitingClan() {
		try (var con = ConnectionFactory.getInstance().getConnection();
			var ps = con.prepareStatement("DELETE FROM siege_clans WHERE castle_id=? and type = 2")) {
			ps.setInt(1, getCastle().getResidenceId());
			ps.execute();
	...
	public void removeSiegeClan(int clanId) {
		...
		try (var con = ConnectionFactory.getInstance().getConnection();
			var ps = con.prepareStatement("DELETE FROM siege_clans WHERE castle_id=? and clan_id=?")) {
			ps.setInt(1, getCastle().getResidenceId());
			ps.setInt(2, clanId);
			ps.execute();
	...
	private void loadSiegeClan() {
		...
		try (var con = ConnectionFactory.getInstance().getConnection();
			var ps = con.prepareStatement("SELECT clan_id,type FROM siege_clans where castle_id=?")) {
			getAttackerClans().clear();
			getDefenderClans().clear();
			getDefenderWaitingClans().clear();
	...
	private void saveSiegeClan(L2Clan clan, byte typeId, boolean isUpdateRegistration) {
		...
		try (var con = ConnectionFactory.getInstance().getConnection()) {
			...
			if (!isUpdateRegistration) {
				try (var ps = con.prepareStatement("INSERT INTO siege_clans (clan_id,castle_id,type,castle_owner) values (?,?,?,0)")) {
				...
				try (var ps = con.prepareStatement("UPDATE siege_clans SET type = ? WHERE castle_id = ? AND clan_id = ?")) {
					ps.setInt(1, typeId);
					ps.setInt(2, getCastle().getResidenceId());
