...
public abstract class L2Character extends L2Object implements ISkillsHolder, IDeletable {
	...
	public L2Character(int objectId, L2CharTemplate template) {
		...
		if (isDoor()) {
			...
			_calculators = new Calculator[NUM_STATS];
		}
	 ...
	 */
	@Override public final boolean isInsideZone(ZoneId zone) {
	...
	public void teleToLocation(int x, int y, int z, int heading, int instanceId, int randomOffset) {
		if (this instanceof L2PcInstance) {
			TerminateReturn terminate = EventDispatcher.getInstance().notifyEvent(new OnPlayerUseTeleportToLocation((L2PcInstance) this, new Location(x, y, z, heading, instanceId)),Containers.Players(), TerminateReturn.class);
			if (terminate !=null && terminate.terminate()) {
				this.sendMessage("You are not allowed to teleport.");
				return;
			}
		}
	...
	public void doAttack(L2Character target) {
		...
		try {
			...
			target.getAttackByList().add(this);
	...
	private void beginCast(Skill skill, boolean simultaneously, L2Character target, L2Object[] targets) {
		...
		if (isPlayer() && !skill.isAbnormalInstant()) {
			...
			switch (magicId) {
				...
				case 2046 -> sm = SystemMessage.getSystemMessage(SystemMessageId.SUMMON_A_PET); // Wolf Collar
				default -> {
		...
		if (skillAnimTime > 0) {
			...
			if (simultaneously) {
				...
				_skillCast = ThreadPoolManager.getInstance().scheduleEffect(mut, (int) skillAnimTime - 370);
			}
		} else {
			mut.setSkillTime(0);
	...
	@Override public CharKnownList getKnownList() {
	...
	public final void startFakeDeath() {
		...
		broadcastPacket(new ChangeWaitType(this, ChangeWaitType.WT_START_FAKE_DEATH));
	}
	...
	public final void stopFakeDeath(boolean removeEffects) {
		...
		broadcastPacket(new ChangeWaitType(this, ChangeWaitType.WT_STOP_FAKE_DEATH));
	 ...
	 */
	@Override public void onForcedAttack(L2PcInstance player) {
	 ...
	 */
	@Override public Map<Integer, Skill> getSkills() {
		return _skills;
	}
	 ...
	 */
	@Override public int getSkillLevel(int skillId) {
	 ...
	 */
	@Override public final Skill getKnownSkill(int skillId) {
			...
			for (L2Object obj : targets) {
				...
				if (npc().raidCurse() && ((target.isRaid() && target.giveRaidCurse() && (getLevel() > (target.getLevel() + 8))) || (!skill.isBad() && (targetsAttackTarget != null) && targetsAttackTarget.isRaid() && targetsAttackTarget.giveRaidCurse() && targetsAttackTarget.getAttackByList().contains(target) && (getLevel() > (targetsAttackTarget.getLevel() + 8))) || (!skill.isBad() && (targetsCastTarget != null) && targetsCastTarget.isRaid() && targetsCastTarget.giveRaidCurse() && targetsCastTarget.getAttackByList().contains(target) && (getLevel() > (targetsCastTarget.getLevel() + 8))))) {
					...
					if (curseSkill != null) {
						...
						LOG.warn("Skill ID {} level {} is missing in DP!", curse.getId(), curse.getLevel());
					}
					return;
				}
	...
	@Override public void removeStatusListener(L2Character object) {
	...
	@Override public Queue<AbstractEventListener> getListeners(EventType type) {
