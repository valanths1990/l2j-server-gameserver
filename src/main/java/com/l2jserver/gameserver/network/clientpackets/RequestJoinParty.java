...
public final class RequestJoinParty extends L2GameClientPacket {
	...
	@Override protected void runImpl() {
		...
		TerminateReturn terminator = EventDispatcher.getInstance().notifyEvent(new OnPlayerPartyRequest(requestor, target), Containers.Players(), TerminateReturn.class);
		if (terminator != null && terminator.abort()) {
			requestor.sendMessage("You are not allowed to join a Party!");
		}
